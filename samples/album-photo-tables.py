...
databases_cursor = databases.cursor()
 
# Check if database exists
databases_cursor.execute(f"SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{RDS_DB_NAME}'")
database = databases_cursor.fetchone()

# Create database if does not exists
if not database:
    databases_cursor.execute(f"CREATE DATABASE {RDS_DB_NAME}")
    database_creation = databases_cursor.fetchone()
    print(f"Database created: {database_creation}")
